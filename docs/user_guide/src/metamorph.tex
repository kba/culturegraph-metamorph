...
\begin{figure}[htp]
\centering
\includegraphics[width=.80\textwidth]{figures/readerwriter}
...
The Metamorph object acts as a pipe element in the data stream. See also figure \ref{fig:readerwriter} and the explanation in section \ref{pipeline}. This means that we must wire it to a data source (or reader) and a data sink (or writer). Listing \ref{codepipeline1} shows how. First all elements of the processing chain are created.
...
\subsection{Splitting Metamorph Definitions for Reuse}
...
\subsection{Testing Framework for Metamorph Definitions}
